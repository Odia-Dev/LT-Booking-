      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        ...
        {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {vehicles.map(v => (
                <button 
                  ...
                  className="group bg-white p-6 sm:p-8 rounded-2xl sm:rounded-[2rem] border border-gray-100 hover:border-[#E60023] transition-all text-left shadow-sm hover:shadow-xl"
                >
                  <div className="aspect-video rounded-xl sm:rounded-2xl overflow-hidden mb-6 bg-gray-50">
                    <img src={v.thumbnailImage || v.image} className="w-full h-full object-contain p-4 group-hover:scale-105 transition-transform duration-500" alt={v.name} />
                  </div>
                  <h3 className="font-bold text-xl sm:text-2xl text-gray-900 uppercase italic leading-tight">{v.name}</h3>
          ...
          <div className="flex flex-col lg:flex-row gap-8 sm:gap-12 items-start">
            ...
            {/* LEFT: Customer Form */}
            <div className="w-full lg:w-[60%] space-y-8 sm:space-y-12">
               <div className="space-y-2">
                 <h1 className="text-3xl sm:text-4xl font-black text-gray-900 uppercase italic leading-tight">Confirm Reservation</h1>
                 <p className="text-sm sm:text-base text-gray-500 font-medium">Please provide your details to secure your Toyota.</p>
               </div>
               ...
               {/* Trust Section */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 pt-12 border-t border-gray-200">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center text-[#E60023] shrink-0">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-gray-500">Laxmi Toyota BAM</p>
                    </div>
                  </div>
               </div>
            </div>
...
function SummaryCard({ vehicle, variant, color, interior, amount }) {
  return (
    <div className="bg-white rounded-2xl sm:rounded-[2rem] border border-gray-100 shadow-xl overflow-hidden p-6 sm:p-8 space-y-6 sm:space-y-8 animate-in slide-in-from-bottom-8 sm:slide-in-from-right-8 duration-700">
       <div className="flex justify-between items-center">
         <h3 className="text-lg sm:text-xl font-bold text-gray-900 uppercase italic">Booking Summary</h3>
       ...
       <div className="aspect-video bg-gray-50 rounded-xl sm:rounded-2xl overflow-hidden">
         <img src={vehicle.thumbnailImage || vehicle.image} className="w-full h-full object-contain p-4" alt={vehicle.name} />
       </div>
       ...
       <div className="space-y-4 sm:space-y-6">
         <div className="flex justify-between items-end border-b border-gray-50 pb-4">
            <div>
               ...
               <p className="font-bold text-base sm:text-lg text-gray-900 uppercase">{vehicle.name}</p>
            </div>
            <div className="text-right">
               <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Variant</p>
               <p className="font-bold text-xs sm:text-sm text-gray-900">{variant || 'V MT'}</p>
            </div>
         </div>

         <div className="grid grid-cols-2 gap-4 sm:gap-8 border-b border-gray-50 pb-4">
            <div>
               <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Exterior</p>
               <p className="font-bold text-[10px] sm:text-[11px] text-gray-900">{color || 'Platinum White'}</p>
            </div>
            <div>
               <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Interior</p>
               <p className="font-bold text-[10px] sm:text-[11px] text-gray-900">{interior || 'Premium Black'}</p>
            </div>
         </div>
         ...
         <div className="pt-6 border-t-2 border-dashed border-gray-100 flex justify-between items-center">
            <p className="text-[10px] sm:text-xs font-black text-gray-900 uppercase tracking-widest">Booking Amount</p>
            <p className="text-2xl sm:text-3xl font-black text-[#E60023]">₹{(parseInt(amount) || 50000).toLocaleString()}</p>
         </div>
       </div>
    </div>
  );
